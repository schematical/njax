    ...
    var ObjectId = app.mongoose.Types.ObjectId;
     var route = app.njax.routes.accesstoken = {
        ...
        render_remove:function(req, res, next){
            res.render('model/accessToken_detail', res.locals.accessToken);
        },
        ...
        populate_list_query:function(req, res, next){
			if(!req._list_query){
				if(req._list_query === false){
                    ...
                    if(req.query.token){
                        var escpaedField = app.njax.helpers.regex.escape(req.query.token);
						req._list_query['token'] =  { $regex: new RegExp('.*' + escpaedField + '', 'i') };
                    }
            ...
            async.series([
                function(cb){
                        ...
                        if(req.query.$orderby){
                            ...
                            return app.model.AccessToken.aggregate(
                                agg_query
                            ).exec(function(err, _accessTokens_data){
                                if(err) return next(err);
                                res.bootstrap('accessTokens', _accessTokens_data);
                                return next();
                            });
            ...
            if(!req.accessToken){
                ...
                //return next(new Error('AccessToken not found'));
					...
					if(!_.isUndefined(req.body.token)){
                        var token = xssFilters.inHTMLData(req.body.token);
