	...
	return njax_tags = {
		...
		queryTags:function(query, callback){
			app.model.Tag.find(
				...
				null,
				/*['value', 'entity_id', 'type', 'sub_type', 'entity_type', 'entity_url', '_entity_name'],*/
				{'group': 'value'},
				function(err, tags) {
		...
		query:function(query, callback){
			...
			return app.model.Tag.aggregate(
				...
				//Now the tricky part - Populating the data.. or should we save that for an on tag basis
				return  callback(null, tags);//rData);
			});
		...
		render:function(event, req, res){
			...
			if(_.isArray(event)){
				...
				//TODO:Render it
				html += njax_tags.search_compiled_tpl[partial].render(event.data, res.locals.partials);
			}
			return html;
		}
	}
