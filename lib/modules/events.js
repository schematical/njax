...
module.exports = function(app){
	...
	return njax_events = {
		...
		queryNewsfeed:function(data){
			...
			if(data.page){
				query_extra = data.page * EVENTS_PER_PAGE
			}
			if(!query.$and){
				query.$and = [];
			}
		...
		render:function(event, req, res){
			...
			if(_.isArray(event)){
				...
				console.log("Looking  Partial For: ", event.event_namespace, njax_events.event_tpl[event.event_namespace]);
				/*console.log("Event: ", event.event_namespace, partial);*/
