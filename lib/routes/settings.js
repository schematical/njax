    ...
    var route = app.njax.routes.settings = {
						...
						if(iframe) {
							...
							switch (iframe.action) {
                                case('application_link'):
                                    iframe.is_application_link = true;
                                    iframe.app_url = applications[i].app_url;
                                    break;
								case('external_link'):
									iframe.is_external_link = true;
									break;
								case('relative_link'):
									iframe.is_relative_link = true;
									break;
								case('tab'):
									iframe.is_tab = true;
									break;
        ...
        middleware:function(page_iframe){
            return function(req, res, next){
                ...
                var tabs = _.clone(app.njax.routes.settings._iframe_cache[page_iframe]);

				res.bootstrap('tabs', tabs);
				return next();
            }
        },
        replaceTabUrlWithLocals:function(tabs, boostrap){
            var rTabs = [];
            for(var i in tabs){
                var tab = _.clone(tabs[i]);
                switch(tab.action){
                    case('application_link'):
                        tab.url = tab.app_url + route.replaceWithLocals(tab.url, boostrap);
                        console.log('application_link', tab.url);
                        break;
                    case('external_link'):
                        tab.url = route.replaceWithLocals(tab.url, boostrap);
                        break;
                    case('relative_link'):
                        tab.url = route.replaceWithLocals(tab.url, boostrap);
                        break;
        ...
        replaceWithLocals:function(url, bootstrap){
            var pos = 0;
            while (url && (pos !== -1)) {

                pos = url.substr(pos).indexOf(':');

                if(url && (pos !== -1)){

                    end_pos = url.substr(pos).indexOf('/');
                    section = url.substr(pos +1, end_pos -1 );
                    //console.log('SECTION', ':' + section, url, pos, end_pos);
                    ...
                    for (var ii in parts) {
                        ...
                        if(ii == 0) {
                            data = bootstrap[key];
                           /* console.log("KEYS:", Object.keys(bootstrap));
                            console.log("data 0:", key, bootstrap[key], _.isString(key));*/
                        }else{
                            if(data) {
                                data = data[key];
                            }
                    ...
                    if(_.isObject(data) && data.namespace){
                        data = data.namespace;
                    }
                    //console.log("data2:", data);
                    if(_.isString(data)){
                        url = url.replace(':' + section, data)
                        //console.log("AFTER_REPLACE:", url);
                        pos += data.length -1;
                    }else{
                        throw new Error("Impropperly formatted Url: " + url);
                        url = null;
                        pos = -1;
                    }
                }
        ...
        route:function(page_iframe){
            ...
            return [
                ...
                route.render_settings
            ]

        },
        render_settings:function(req, res, next){
        ...
        render_iframe:function(req, res, next){
            res.render('iframe');
        },
        middleware_iframe_parent:function(page_iframe){
            ...
            return function(req, res, next){
                ...
                //console.log(req.njax.tab_id, req.application.iframes[req.njax.tab_id].iframe_type + ' == ' +page_iframe);
                ...
                //njax_payload = decodeURIComponent(escape(njax_payload));
        ...
        render_basic: function(req, res, next){
