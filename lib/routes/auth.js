    ...
    var route = app.njax.routes.auth = {
        ...
        init:function(){
            ...
            app.use(function(req, res, next){
                ...
                if(req.user && app.njax.env_config.super_admin_emails && _.contains(app.njax.env_config.super_admin_emails, req.user.email)){
			...
			app.options(app.njax.env_config.register_url, function(req, res, next){
				res.send("OK");
			});
			app.options(app.njax.env_config.auth_url, function(req, res, next){
				res.send("OK");
			});
            app.get(app.njax.env_config.register_url, [
            ...
            app.post(
                app.njax.env_config.register_url,
                [
                    app.njax.force_https(),
					route.populate_redirect_url,
      ...
      /*  auth://function(req, res, next){
        ...
        auth:function(req, res, next){
            ...
            async.series([
                ...
                function(cb){
                    return app.njax.api.create_token(req.user, app.njax.env_config.core.app, function(err, access_token){
                        if(err) return next(err);
                        res.cookie('access_token', access_token, app.njax.env_config.cookie);
                        return cb();
                    });
                }
            ],
        ...
        auth_error_check:function(tpl){
			return function(err, req, res, next){
				...
				res.bootstrap('error', { message: err.message });
				res.render(tpl, req.error);//'auth'
			}
		},
        ...
        logout:function(req, res, next) {
            req.logout();
            res.clearCookie('access_token', app.njax.env_config.cookie);
            req.session.destroy();
        ...
        validate_register:function(req, res, next){
            ...
            if(_.contains(app.njax.env_config.illegal_namespaces, req.body.namespace)){
        ...
        broadcast_register:function(req, res, next){
            ...
            app.njax.broadcast(
                ...
                'auth.register',
                res._bootstrap,
                (req.active_application && req.active_application.namespace) || app.njax.env_config.core.app
            );
            return next();
        },
