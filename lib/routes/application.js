    ...
    function render_edit(req, res, next){
        async.series([
            ...
            function(cb){
                ...
                res.locals.app_levels = [];
                var levels = app.model.Application.levels();
                for(var i in levels){
                    res.locals.app_levels.push({
                        name:levels[i],
                        value:levels[i],
                        selected: (req.application.type == levels[i])
                    })
                }
                return cb();
            },
