        ...
        populate_list_query:function(req, res, next){
			if(!req._list_query){
				if(req._list_query === false){
                    ...
                    if(req.query.email){
                        var escpaedField = app.njax.helpers.regex.escape(req.query.email);
						req._list_query['email'] =  { $regex: new RegExp('.*' + escpaedField + '', 'i') };
                    }
                    ...
                    if(req.query.forgot_pass_code){
                        var escpaedField = app.njax.helpers.regex.escape(req.query.forgot_pass_code);
						req._list_query['forgot_pass_code'] =  { $regex: new RegExp('.*' + escpaedField + '', 'i') };
                    }
            ...
            if(!req.account){
                req.account = new app.model.Account({
                    ...
                    if(!req.body.namespace && req.body.name){
                        req.account.namespace = app.njax.helpers.toNamespace(req.body.name);
                    }
            ...
            if(!req.account){
                ...
                //return next(new Error('Account not found'));
					...
					if(!_.isUndefined(req.body.namespace)){
                    	...
                    	req.account.namespace = namespace;
					}
                
            
                
					if(!_.isUndefined(req.body.active)){
                        var active = xssFilters.inHTMLData(req.body.active);

                    	req.account.active = active;
					}
                
            
                
					if(!_.isUndefined(req.body.forgot_pass_code)){
                        var forgot_pass_code = xssFilters.inHTMLData(req.body.forgot_pass_code);

                    	req.account.forgot_pass_code = forgot_pass_code;
					}
