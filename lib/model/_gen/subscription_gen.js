...
module.exports = function(app){
    ...
    subscriptionSchema.virtual('uri').get(function(){
				...
				if(!this.account){
					...
					var parent_uri =  (this.account.uri || (this._parent_uri));
				}
                return parent_uri + '/subscriptions/' + this._id;
            ...
            if(!this._parent_uri){
            	if(this.account){
            		var _this = this;
						...
						return app.model.Account.findOne({ _id: this.account }).exec(function(err, account){
							if(err) return next(err);
							if(!account){
								return next(new Error("No account found when trying to populate _parent_uri. Either find it or manually populate the _parent_uri."));
							}
							_this._parent_uri = account.uri;
							return next();
						});
					...
					return next(new Error("Missing Parent Field: account!"));
