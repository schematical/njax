...
module.exports = function(app){
    var njax_api =  {
                ...
                async.series([
                    ...
                    function(cb){
                        ...
                        if(!origin){
                            ...
                            for(var i in app.njax.super_applications){
                                if(app.njax.super_applications[i].domain == origin){
                                    allow_origin = app.njax.super_applications[i].domain;
                                }
                            }
                        }
                        if(!allow_origin){
                            return res.send("This Origin is not allowed by 'Access-Control-Allow-Origin'");
                        }
        ...
        init_super_apps:function(app, next){
            if(app.njax.super_applications){
                return next();
            }
            app.model.Application.find({ level:'SUPER' }).exec(function(err, applications){
                app.njax.super_applications = applications;
                for(var i in app.njax.super_applications){
                    console.log(app.njax.super_applications[i].namespace + '_www_url', app.njax.super_applications[i].app_url);
                    app.locals[app.njax.super_applications[i].namespace + '_www_url'] = app.njax.super_applications[i].app_url;
                }
                if(err) return next(err);
